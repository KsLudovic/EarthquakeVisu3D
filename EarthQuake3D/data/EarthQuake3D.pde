...
//Earthquake data
Table quakes;
color quakeColor = color(255, 0, 50);
double maxMag = 0;
...
void draw() {
  ...
  if (!loaded) {        //Loading: show loader
    ...
    fill(255);
    textSize(16);
    text("a key for 30 days, b key for 7, c for 1", width/2, 50);
   // text("Number of Earthquakes : "+count, width/2, 150);
    cam.endHUD();  
...
void drawPlace(PVector latlong, color col, float boxHeight) {
  ...
  fill(col, 128);
  box(boxHeight+betterView, 10, 10);
  popMatrix();
...
void loadData() {
  ...
  PImage tex = loadImage("https://eoimages.gsfc.nasa.gov/images/imagerecords/73000/73776/world.topo.bathy.200408.3x5400x2700.jpg");
  ...
  //Load earthquake data and find maximum magnitude by default
...
void initView() {
  ...
  cam.setMinimumDistance(dist/3);
  cam.setMaximumDistance(dist);
  //Reset animation
  animAngle = 0;
  //Done initializing
  init = true;
...
void drawLoader() {
  ...
  text("Use mouse: left drag to rotate, wheel or right drag to zoom, a b c keys to switch datas", width - 20, height - 20);
